...
// Replace numeric preset variables with safe values ("Unified" when the old variables are undefined).
private _bluforPreset = if (isNil "KP_liberation_preset_blufor") then {"Unified"} else {KP_liberation_preset_blufor};
private _opforPreset = if (isNil "KP_liberation_preset_opfor") then {"Unified"} else {KP_liberation_preset_opfor};
private _resPreset = if (isNil "KP_liberation_preset_resistance") then {"Unified"} else {KP_liberation_preset_resistance};
private _civPreset = if (isNil "KP_liberation_preset_civilians") then {"Unified"} else {KP_liberation_preset_civilians};
[format ["Blufor: %1", _bluforPreset], "MISSIONSTART"] call KPLIB_fnc_log;
